...
while cap.isOpened():
    ...
    # print(f'pose" {(poseTrackResult)}')

    #tth
    '''
    lay thong tin person roi rac theo tung frame
    '''
    # def extract_person_info(track_ids, poseTrackResult):
    #     person_info = []
    ...
    # # Example usage:
    # person_info = extract_person_info(track_ids_conform_frame_num, poseTrackResult)
    # for person in person_info:
    #     print("Person ID:", person['id'])
    #     print("Frame Number:", person['num_frame'])
    #     print("Keypoints:", person['keypoints'])
    #     print("Keypoint Scores:", person['keypoint_scores'])
    #     print()

    '''
    lay thong tin day du cua person
    '''
    def extract_person_info(track_ids, poseTrackResult,max_person):
        person_info = []
        for idx, track_id in enumerate(track_ids):
            num_frames = poseTrackResult[idx].shape[1]
            # all_keypoints = []
            # all_keypoint_scores = []
            # 1. Create arrays with a leading dimension of 1 for max_person
            all_keypoints = np.zeros((1, num_frames, 17, 2), dtype=np.float16)
            all_keypoint_scores = np.zeros((1, num_frames, 17), dtype=np.float16)
            for frame_idx in range(num_frames):
                keypoints = poseTrackResult[idx][0, frame_idx].reshape(-1, 3)
                keypoint_scores = keypoints[:, 2]
                keypoints = keypoints[:, :2]
                # 2. Fill data into the first dimension (max_person=1)
                all_keypoints[0, frame_idx] = keypoints
                all_keypoint_scores[0, frame_idx] = keypoint_scores
            ...
            person_info.append({
                ...
                # 'frame_range': (frame_idx - num_frames + 1, frame_idx)
            })
        return person_info
    ...
    max_person = 1
    if count % 5 ==0: 
        person_info = extract_person_info(track_ids_conform_frame_num, poseTrackResult,max_person)
        for person in person_info:
            ...
            print("num_frame", person['num_frames'])
            # print("frame_range", person['frame_range'])
            # print(f'num_frame of ID{person['id']} is {person['num_frames']}')
            # print(f'frame_range of ID{person['id']} is {person['frame_range']}')
            print("----------------------------------------")

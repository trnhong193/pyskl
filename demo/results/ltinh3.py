    ...
    def extract_person_info(track_ids, poseTrackResult, max_person):
        ...
        for idx, track_id in enumerate(track_ids):
            ...
            for frame_idx in range(num_frames):
                ...
                if frame_idx % 5 == 0:
                    if frame_idx != 0:
                        person_info.append({
                            ...
                            'num_frames': frame_idx - start_frame
                        })
                        # Reset arrays
                        all_keypoints = np.zeros((1, num_frames, 17, 2), dtype=np.float16)
                        all_keypoint_scores = np.zeros((1, num_frames, 17), dtype=np.float16)
                ...
                # Fill data into the first dimension (max_person=1)
                all_keypoints[0, frame_idx] = keypoints
                all_keypoint_scores[0, frame_idx] = keypoint_scores

            # Append remaining frames if not multiple of 5
            if num_frames % 5 != 0:
                person_info.append({
                    'id': track_id,
                    'keypoints': all_keypoints,
                    'keypoint_scores': all_keypoint_scores,
                    'num_frames': num_frames - start_frame
                })

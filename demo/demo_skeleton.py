...
def parse_args():
    ...
    parser.add_argument(
        '--pose-config',
        default='/media/ivsr/data/pyskl/demo/pose.py',
        help='human pose estimation config file path (from mmpose)')
    parser.add_argument(
        '--pose-checkpoint',
        default='/media/ivsr/data/pyskl/demo/yoloxpose_s_8xb32-300e_coco-640-56c79c1f_20230829.pth',
        help='human pose estimation checkpoint file/url')
    parser.add_argument(
        '--det-score-thr',
        type=float,
...
def main():
    ...
    ts8 = timer()
    print('444444', ts8 - ts7)
    ts9 = timer()
    model = init_recognizer(config, args.checkpoint, args.device)
    ts10 = timer()
    print('555555', ts10 - ts9)
    # Load label_map
    ts11    = timer()
    label_map = [x.strip() for x in open(args.label_map).readlines()]
    ts12 = timer()
    print('6666666',ts12-ts11)
    ts13 = timer()
    ...
    # ts20 = timer()
    # print('aaaaaaa',ts20-ts19)

    # ts21 = timer()
        action_label = label_map[results[0][0]]
    # ts22 = timer()
    # print('bbbbb', ts22- ts21)

    ts23=timer()
    pose_model = init_pose_model(args.pose_config, args.pose_checkpoint,
                                 args.device)
    ts24=timer()
    print('cccccc', ts24-ts23)
    ...
    ts26=timer()
    print('ddddd', ts26-ts25)
    ts27=timer()
    for frame in vis_frames:
        cv2.putText(frame, action_label, (10, 30), FONTFACE, FONTSCALE,
                    FONTCOLOR, THICKNESS, LINETYPE)
    ts28=timer()
    print('eeeeeeee', ts28-ts27)
    ts29=timer()
    vid = mpy.ImageSequenceClip([x[:, :, ::-1] for x in vis_frames], fps=24)
    ts30=timer()
    print('ffffff', ts30-ts29)
    ts31=timer()
